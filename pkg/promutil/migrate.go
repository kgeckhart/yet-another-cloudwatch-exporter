		...
		for _, metric := range result.Data {
			...
			for _, statistic := range statisticsRepresentedInMetric {
				...
				if metric.GetMetricDataResult != nil {
					...
					dataPoint, ts, err := getDatapointFromStaticResults(metric.MetricConfig.Name, metric.GetMetricStatisticResult.Datapoints, statistic)
					if err != nil {
						return nil, nil, err
					}
					timestamp = ts
					if dataPoint == nil {
						if *metric.MetricConfig.NilToZero {
							exportedDatapoint = 0
						} else {
...
func getDatapointFromStaticResults(metricName string, datapoints []*model.Datapoint, statistic string) (*float64, time.Time, error) {
	...
	for _, datapoint := range sortByTimestamp(datapoints) {
		switch {
